...
# TransformerEncoder ablation (time-only)
HIDDEN_CHANNELS = 64
NHEAD = 8
FF_DIM = 128
ENC_LAYERS = 2
...
# OUTPUT DIR
# ----------------------------
OUTPUT_ROOT = "metrics_outputs_timeonly_trans_snapshot"
os.makedirs(OUTPUT_ROOT, exist_ok=True)

GRID_RESULTS_FILE = "grid_results_timeonly_trans_snapshot.csv"
GRID_SUMMARY_FILE = "grid_summary_timeonly_trans_snapshot.csv"
GRID_PER_TOKEN_FILE = "grid_per_token_results_timeonly_trans_snapshot.csv"
...
print("\n--- Loader sizes ---")
tmp_train_loader, tmp_val_loader, tmp_test_loader = make_loaders(SEEDS[0])
print(f"Train snapshots: {len(train_snapshots)} | batches: {len(tmp_train_loader)}")
print(f"Val   snapshots: {len(val_snapshots)}   | batches: {len(tmp_val_loader)}")
print(f"Test  snapshots: {len(test_snapshots)}  | batches: {len(tmp_test_loader)}")
...
# ----------------------------
# 7) MODEL: TransformerEncoder time-only (ablation)
# ----------------------------
class TimeOnlyTransformer(nn.Module):
    def __init__(
        ...
        self.in_proj = nn.Linear(num_features, hidden_channels)
        ...
        # learnable positional embedding as in ST-GNN
        self.pos_embedding = nn.Parameter(torch.randn(1, window_size, hidden_channels))

        self.out = nn.Linear(hidden_channels, 1)

    def forward(self, x):  # x: [B, W, F]
        h = self.in_proj(x)                 # [B, W, H]
        h = h + self.pos_embedding          # broadcast on batch
        h = self.encoder(h)                 # [B, W, H]
        last = h[:, -1, :]                  # [B, H]
        logits = self.out(last).squeeze(-1) # [B]
...
def run_one(seed: int):
    ...
    print(f"\n--- Start Training (Transformer time-only, snapshot-level) ({EPOCHS} epochs) | Seed {seed} ---")
    ...
    # ----------------------------
    print(f"\n\n--- Final Results (Transformer time-only, snapshot-level) | Seed {seed} ---")
...
def run_multi_seed():
    ...
    if len(PER_TOKEN_ROWS) > 0:
        ...
        pd.DataFrame(columns=[
            "seed", "hidden_channels", "nhead", "ff_dim", "enc_layers", "dropout", "best_val_threshold",
            "token", "n_obs", "pumps", "precision", "recall", "f1"
        ]).to_csv(GRID_PER_TOKEN_FILE, index=False)

    summary = (
        df_res
        .groupby(["hidden_channels", "nhead", "ff_dim", "enc_layers", "dropout"], as_index=False)
        .agg(
    ...
    print(f"FF_DIM = {int(best['ff_dim'])}")
    print(f"ENC_LAYERS = {int(best['enc_layers'])}")
...
if __name__ == "__main__":
    print("\n--- Running Multi-Seed Time-Only Transformer Experiment ---")
    run_multi_seed()